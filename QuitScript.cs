...
//	public GameObject quitobject;
    ...
    void Update()
    {
 		//pressing back arrow or escape quits the game
		/* Check input for the first time
		 * then wait 3 seconds. If the player presses the
		 * button one more time, quit the game. */
	    if (Input.GetKeyDown(KeyCode.Escape) && !clickedBefore){
	        Debug.Log("Back Button pressed for the first time");
	        //Set to false so that this input is not checked again. 
			//It will be checked in the coroutine function instead.
	        clickedBefore = true;
